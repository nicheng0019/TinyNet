...
class ConvNetBuilder(object):
  ...
  def _conv2d_impl(self, input_layer, num_channels_in, filters, kernel_size,
    ...
    if self.use_tf_layers:
      ...
      weights = self.get_variable('weights', weights_shape,
  ...
  def _separable_conv2d_impl(self, input_layer, num_channels_in, num_channels_out, channel_multiplier, kernel_size,
                   strides, padding, kernel_initializer):
    if self.use_tf_layers:
      # TODo: add tf layers for separable_conv2d
      print('error')
      assert(False)
      # return conv_layers.conv2d(input_layer, filters, kernel_size, strides,
      #                           padding, self.channel_pos,
      ...
      depthwise_shape = [kernel_size[0], kernel_size[1], num_channels_in, channel_multiplier]
      depthwise = self.get_variable('depthwise_weights', depthwise_shape,
                                  self.variable_dtype, self.dtype,
                                  regularizer=self.regularizer,
                                  initializer=kernel_initializer)
      #depthwise only
      if num_channels_out is None:
        return tf.nn.depthwise_conv2d_native(input_layer, depthwise, strides, padding,
                          data_format=self.data_format)

      #depthwise follow with pointwise
      pointwise_shape = [1, 1, channel_multiplier * num_channels_in, num_channels_out]
      pointwise = self.get_variable('pointwise_weights', pointwise_shape,
                                  self.variable_dtype, self.dtype,
                                  regularizer=self.regularizer,
                                  initializer=kernel_initializer)
      return tf.nn.separable_conv2d(input_layer, depthwise, pointwise, strides, padding,
                          data_format=self.data_format)
  ...
  def separable_conv(self,
    ...
    if scope is not None:
      ...
      name = 'separable_conv' + str(self.counts['separable_conv'])
      self.counts['separable_conv'] += 1
    with tf.variable_scope(name):
      strides = [1, d_height, d_width, 1]
      if self.data_format == 'NCHW':
        strides = [strides[0], strides[3], strides[1], strides[2]]
      if mode != 'SAME_RESNET':
        separable_conv = self._separable_conv2d_impl(input_layer, num_channels_in, num_out_channels, channel_multiplier,
                                 kernel_size=[k_height, k_width],
                                 strides=[d_height, d_width], padding=mode,
                                 kernel_initializer=kernel_initializer)
      else:  # Special padding mode for ResNet models
        if d_height == 1 and d_width == 1:
          separable_conv = self._separable_conv2d_impl(input_layer, num_channels_in,
                                   num_out_channels,
                                   kernel_size=[k_height, k_width],
                                   strides=[d_height, d_width], padding='SAME',
                                   kernel_initializer=kernel_initializer)
      ...
      if not use_batch_norm:
        if bias is not None:
          biases = self.get_variable('biases', [num_out_channels],
                                     ...
                                     initializer=tf.constant_initializer(bias))
          biased = tf.reshape(
              tf.nn.bias_add(separable_conv, biases, data_format=self.data_format),
              separable_conv.get_shape())
        else:
          biased = separable_conv
      else:
        self.top_layer = separable_conv
        self.top_size = num_out_channels
        biased = self.batch_norm(**self.batch_norm_config)
      if activation == 'relu6':
        conv1 = tf.nn.relu6(biased)
  ...
  def batch_norm(self, input_layer=None, decay=0.999, scale=False,
    ...
    if input_layer is None:
      ...
      self.top_size = None
    name = 'BatchNorm' #'batchnorm' + str(self.counts['batchnorm'])
